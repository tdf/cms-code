<li><h2>
<% if Top.getDonatePageLink %><% if InstallType = Full %><a href='{$Top.getDonatePageLink}dl/$Top.Type/$Top.Version/$Top.Lang/$Filename'
<% else_if Type = box    %><a href='{$Top.getDonatePageLink}dl/$Top.Type/$Top.Version/multi/$Filename<% if TotalItems %>?idx=$Pos<% end_if %>'
<% else_if Type = src    %><% if First %><a href='{$Top.getDonatePageLink}dl/$Top.Type/$Top.Version/all/$Filename?idx=1'<% else %><a href='http://download.documentfoundation.org/$Fullpath'<% end_if %>
