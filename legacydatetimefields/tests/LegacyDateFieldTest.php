 ...
 * @subpackage tests
 */
class LegacyDateFieldTest extends SapphireTest {
	
	function testDMYFormat() {
		/* We get YYYY-MM-DD format as the data value for DD/MM/YYYY input value */
		$dateField = new LegacyDateField('Date', 'Date', '04/03/2003');
		...
		$dateField2 = new LegacyDateField('Date', 'Date', '4/3/03');
	...
	function testYMDFormat() {
		...
		$dateField = new LegacyDateField('Date', 'Date', '2003/03/04');
		$this->assertEquals($dateField->dataValue(), '2003-03-04');
		
		/* Even if input value hasn't got leading 0's in it we still get the correct data value */
		$dateField2 = new LegacyDateField('Date', 'Date', '2003/3/4');
